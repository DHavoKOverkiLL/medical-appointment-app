...
@Component({
  ...
  styleUrls: ['./admin-account-settings.component.scss']
})
export class AdminAccountSettingsComponent implements OnInit {
  ...
  saveSettings(): void {
    ...
    this.dashboardApi.updateMyAccountSettings(payload).subscribe({
      next: response => {
        this.successMessage = response.message || this.translate.instant('adminAccountSettings.messages.updated');
      ...
      error: err => {
        this.errorMessage = err?.error?.message || err?.error || 'adminAccountSettings.errors.updateFailed';
        this.saving = false;
      }
    });
  }
  ...
  private loadAccount(): void {
    ...
    this.dashboardApi.getMyProfile().subscribe({
      ...
      error: () => {
        this.errorMessage = 'adminAccountSettings.errors.loadFailed';
        this.loading = false;
      }
    });
  }
