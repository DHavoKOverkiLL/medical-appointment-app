...
@Component({
  selector: 'app-patient-account-settings',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './patient-account-settings.component.html',
  styleUrls: ['./patient-account-settings.component.scss']
})
export class PatientAccountSettingsComponent implements OnInit {
  ...
  get roleLabelKey(): string {
    const role = this.roleName.trim().toLowerCase();
  ...
  saveSettings(): void {
    ...
    this.dashboardApi.updateMyAccountSettings(payload).subscribe({
      next: response => {
        this.successMessage = response.message || this.translate.instant('accountSettings.messages.updated');
      ...
      error: err => {
        this.errorMessage = err?.error?.message || err?.error || 'accountSettings.errors.updateFailed';
        this.saving = false;
      }
    });
  }
  ...
  private loadAccount(): void {
    ...
    this.dashboardApi.getMyProfile().subscribe({
      ...
      error: () => {
        this.errorMessage = 'accountSettings.errors.loadFailed';
        this.loading = false;
      }
    });
  }
